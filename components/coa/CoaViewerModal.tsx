  ...
  return createPortal(
    <AnimatePresence>
      <motion.button
        key="coa-backdrop"
        type="button"
        className="fixed inset-0 z-[600] bg-lab-base/85 backdrop-blur-sm"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      ...
      <motion.div
        ...
        className="fixed inset-3 z-[610] flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-lab-surface shadow-card sm:inset-6 md:inset-10 lg:inset-16"
        ...
        <div className="min-h-0 flex-1 overflow-auto bg-lab-base/40 p-2 sm:p-4">
          {pdf ? (
            <iframe
              ...
              className="h-[70vh] min-h-[420px] w-full rounded-xl border border-white/5 bg-white"
            />
          ) : (
