...
const filename = 'media__1780290160073.jpg';

async function main() {
  const { data: products } = await supabase.from('products').select('slug, name').ilike('slug', '%glow%');
  console.log("Found products matching 'glow':", products);
  
  let targetSlug = 'glow-10mg-70mg-10mg';
  // check if 'glow-bpc-157-ghk-cu-tb-500-blend' exists
  const exactMatch = products.find(p => p.slug === 'glow-bpc-157-ghk-cu-tb-500-blend');
  if (exactMatch) targetSlug = exactMatch.slug;
  
  console.log("Using slug:", targetSlug);
